...
# Initialization {{{
source $HOME/.dot-files/themes/lib/async.zsh
...
ZSH_THEME_GIT_PROMPT_CLEAN="%F{cyan}%B)%F{green}%B✔%f%b"
ZSH_THEME_GIT_PROMPT_DIRTY="%F{cyan}%B)%F{yellow}%B!%f%b"
ZSH_THEME_GIT_PROMPT_PREFIX=" %F{cyan}%Bgit:(%F{magenta}%B"
...
function terraform_workspace {
  [[ $(tf_prompt_info) == "" ]] && return
  echo " %{$fg_bold[cyan]%}tf:(%{$fg[magenta]%}$(tf_prompt_info)%{$fg_bold[cyan]%})%{$reset_color%}"
}

function k8s_context {
  [[ $ZSH_KUBECTL_PROMPT == "" ]] && return
  echo " %{$fg_bold[cyan]%}k8s:(%{$fg[magenta]%}$ZSH_KUBECTL_PROMPT%{$fg_bold[cyan]%})%{$reset_color%}"
}

ZSH_THEME_AWS_PREFIX=" %{$fg_bold[cyan]%}aws:(%{$fg[magenta]%}"
ZSH_THEME_AWS_SUFFIX="%{$fg_bold[cyan]%})%{$reset_color%}"


#RPROMPT='%{$fg[cyan]%}($ZSH_KUBECTL_PROMPT)%{$reset_color%}'


PROMPT+='$(terraform_workspace)$(aws_prompt_info)$(k8s_context)$DRACULA_GIT_STATUS'
# Ensure effects are reset
PROMPT+='
%(1V:%F{yellow}:%(?:%F{green}:%F{red}))${DRACULA_ARROW_ICON}%f%b '
